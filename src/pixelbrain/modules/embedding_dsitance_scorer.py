...
class ScoringStrategy(ABC):
    ...
    """
    @abstractmethod
    def score(self, tested_vector: np.ndarray, compare_to_vectors: List[np.ndarray]) -> float:
        pass
...
class AverageKNearestStrategy(ScoringStrategy):
    ...
    def score(self, tested_vector: np.ndarray, compare_to_vectors: List[np.ndarray]) -> float:
        distances = np.array([np.linalg.norm(tested_vector - v) for v in compare_to_vectors])
        k = min(self.k, len(distances))  # Use min to handle cases with fewer than k vectors
        nearest_indices = np.argsort(distances)[:k]
        average_vector = np.mean([compare_to_vectors[i] for i in nearest_indices], axis=0)
...
class EmbeddingDistanceScorerModule:
    ...
    def score_images(self, n_closest_vectors: int = 40):
        ...
        for image in tested_images:
            ...
            closest_meta, _ = self.database.query_vector_field(self.compare_to_field, tested_vector, n_results=n_closest_vectors)
            closest_vectors = [self.database.get_field(image['_id'], self.compare_to_field) for image in closest_meta]
            if len(closest_vectors) == 0 or not isinstance(closest_vectors[0], np.ndarray):
