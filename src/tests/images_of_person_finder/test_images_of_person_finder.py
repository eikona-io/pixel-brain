...
def images_of_person_finder_run(path_to_person_image: str):
    database = Database()
    data = DataLoader(
        f"{PIXELBRAIN_PATH}/src/tests/images_of_person_finder/data", database, batch_size=1
    )
    ...
    with DeleteDatabaseAfterTest(database):
        ...
        matched_persons = {image["image_path"]: image["matched_person"] for image in metadata}
...
def test_images_of_person_finder():
    path_to_person_image = f"{PIXELBRAIN_PATH}/src/tests/images_of_person_finder/1_7.jpg"
